...
# Table 2 ####
fit.ppsv23 <- glm(ppsv23_vacci ~ group + age_group + sex + occupation + area + maristatus + education +income + living + distance, 
           data = data, family = binomial(link = "logit"))
tbl_merge(list(tbl_regression(fit.ppsv23, exponentiate = T)), 
          tab_spanner = c("ppsv23 aor all")) %>% as_flex_table() %>% flextable::save_as_docx(path = "ppsv23 aor.all.docx")
tbl_merge(list(tbl_regression(glm(ppsv23_vacci ~ group, data = data, family = binomial(link = "logit")), exponentiate = T)), 
          tab_spanner = c("ppsv23 cor all")) %>% as_flex_table() %>% flextable::save_as_docx(path = "ppsv23 cor.all.docx")

fit.flu <- glm(flu_vacci ~ group + age_group + sex + occupation + area + maristatus + education +income + living + distance, 
            data = data, family = binomial(link = "logit"))
tbl_merge(list(tbl_regression(fit.flu, exponentiate = T)), 
          tab_spanner = c("flu aor all")) %>% as_flex_table() %>% flextable::save_as_docx(path = "flu aor.all.docx")
tbl_merge(list(tbl_regression(glm(flu_vacci ~ group, data = data, family = binomial(link = "logit")), exponentiate = T)), 
          tab_spanner = c("flu cor all")) %>% as_flex_table() %>% flextable::save_as_docx(path = "flu cor.all.docx")
...
# incomelevel_aOR
fit.incomel <- glm(ppsv23_vacci ~ group + age_group + sex + occupation + area + maristatus + 
                     education + living + distance, data = data[(data$incomelevel == "lower"), ], family = binomial(link = "logit"))
fit.incomeh <- glm(ppsv23_vacci ~ group + age_group + sex + occupation + area + maristatus + 
...
# incomelevel_cOR
tbl_merge(list(
  tbl_regression(glm(ppsv23_vacci ~ group, data = data[data$incomelevel == "lower", ], family = binomial(link = "logit")), exponentiate = T) |>
    modify_column_merge(pattern = "{estimate} ({conf.low}, {conf.high})", rows = !is.na(estimate)),
  tbl_regression(glm(ppsv23_vacci ~ group, data = data[data$incomelevel == "higher", ], family = binomial(link = "logit")), exponentiate = T) |>
    modify_column_merge(pattern = "{estimate} ({conf.low}, {conf.high})", rows = !is.na(estimate))), 
  tab_spanner = c("cor income = lower", "cor income = higher")) %>% as_flex_table() %>% flextable::save_as_docx(path = "cor.income.docx")

# living_aOR
fit.withspous<- glm(ppsv23_vacci ~ group + age_group + sex + occupation + area + education + distance + income, 
                    data = data[(data$living == "with spouse"), ], family = binomial(link = "logit"))
fit.notwithspous <- glm(ppsv23_vacci ~ group + age_group + sex + occupation + area +education + distance + income, 
...
# living_cOR
tbl_merge(list(
  tbl_regression(glm(ppsv23_vacci ~ group, data = data[(data$living == "with spouse"), ], family = binomial(link = "logit")), exponentiate = T) |>
    modify_column_merge(pattern = "{estimate} ({conf.low}, {conf.high})", rows = !is.na(estimate)),
  tbl_regression(glm(ppsv23_vacci ~ group, data = data[(data$living == "notwith spouse"), ], family = binomial(link = "logit")), exponentiate = T) |>
    modify_column_merge(pattern = "{estimate} ({conf.low}, {conf.high})",rows = !is.na(estimate))), 
  tab_spanner = c("cor living = with spouse", "cor living = notwith spouse")) %>% as_flex_table() %>% flextable::save_as_docx(path = "cor.living.docx")

# chronic disease_aOR
fit.wcd <- glm(ppsv23_vacci ~ group + age_group + sex + maristatus + occupation + area + education + distance + income + living,
               data = data[data$cdisease == "yes", ], family = binomial(link = "logit"))
fit.ncd <- glm(ppsv23_vacci ~ group + age_group + sex + maristatus + occupation + area + education + distance + income + living, 
...
# chronic disease_cOR
tbl_merge(list(
  tbl_regression(glm(ppsv23_vacci ~ group, data = data[data$cdisease == "yes", ], family = binomial(link = "logit")), exponentiate = T),
  tbl_regression(glm(ppsv23_vacci ~ group, data = data[data$cdisease == "no", ], family = binomial(link = "logit")), exponentiate = T)), 
  tab_spanner = c("cor cdisease = yes", "cor cdisease = no")) %>% as_flex_table() %>% flextable::save_as_docx(path = "cor.cdisease.docx")

# area_aOR
fit.urban <- glm(ppsv23_vacci ~ group + age_group + sex + maristatus + occupation + education + distance + income + living, 
                 data = data[data$area == "urban", ], family = binomial(link = "logit"))
fit.rural <- glm(ppsv23_vacci ~  group + age_group + sex + maristatus + occupation + education + distance + income + living,
...
# area_cOR
tbl_merge(list(
  tbl_regression(glm(ppsv23_vacci ~ group, data = data[data$area == "urban", ], family = binomial(link = "logit")), exponentiate = T),
  tbl_regression(glm(ppsv23_vacci ~ group, data = data[data$area == "rural", ], family = binomial(link = "logit")), exponentiate = T)), 
